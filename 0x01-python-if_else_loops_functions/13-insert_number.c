...
listint *insert_node(listint_t **head, int number)
{
  ...
  /* IF NO LINKED LISTS, INSERT NODE as the only member */
  if (*head == NULL)
    {
      *head = new;
      (*head)->next = NULL;
  ...
  /*if only one node in linked list, do compariosn and insert */
  if ((*head)->next == NULL)
    {
      if ((*head)->n < new->n)
	(*head)->next = new;
  ...
  while (tmp->next != NULL)
    {
      /*if new node num is smalled than first node, insert*/
      if (new-> < tmp->n)
	{
	  new->next = tmp;
	  *head = new;
      ...
      /*if new node sum is the same as an existing node, insert*/
      /* compare previous node and next node, insert in between */

      if (((new->n > tmp->n) && (new-> < (tmp->next)->n)) ||
	  (new->n == tmp->n))
	{
	  new->next = tmp->next;
	  tmp->next = new;
